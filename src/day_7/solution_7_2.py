...
def check_equation(target: int, current_value: int, values_to_use: list[int]) -> list[bool]:
    ...
    concatentation_value = int(str(current_value) + str(values_to_use[0]))

    if len(values_to_use) == 1:
        return [addition_value == target, multiplication_value == target, concatentation_value == target]

    return_value = (
        check_equation(target, addition_value, values_to_use[1:]) +
        check_equation(target, multiplication_value, values_to_use[1:]) +
        check_equation(target, concatentation_value, values_to_use[1:])
    )
